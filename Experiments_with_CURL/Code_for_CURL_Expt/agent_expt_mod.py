...
class Agent(nn.Module):
    ...
    def __init__(self, params):
        ...
        ############################## Added for Experiment context inclusion troubleshooting ############################## 
        self.tt_nei = params['tt_embeddings_entity'] ### added for Experiment 
        self.dtac_embed=params['dtac_embed']
        ############################## Added for Experiment ################################################################ 
        if self.use_entity_embeddings:
            if self.train_entity_embeddings:
                self.entity_embedding = nn.Embedding(self.entity_vocab_size, 2 * self.embedding_size,padding_idx=0) ### added for DTAC
            else:
    ...
    def step(self, next_relations, next_entities, prev_state, prev_relation, query_embedding, current_entities,
        ...
        ############################## Added for Experiment context inclusion troubleshooting ############################## 
        #### changes to include nieghbor 17th oct
        #### Idea is to Read embeddings of all one hop neigbours of given Ticket(T) and compute mean, neigbours are provided as self.tt_nei  in train.py 
        if self.dtac_embed is True:
            #print('here,enbed',self.dtac_embed)
            al=current_entities.cpu().numpy()
            ett=self.entity_embedding(torch.tensor(self.tt_nei[al,:]))
            prev_entity=torch.mean(ett,dim=1)
        #print(prev_entity_n.shape,prev_entity.shape, 'new work')
        #### changes to include nieghbor 17th oct
        ############################## Added for Experiment context inclusion troubleshooting ############################## 
        ...
        ### add test
        #test_entitities= torch.Tensor([x for x in range(75494)]).to(torch.int64)
        test_entitities= torch.Tensor([x for x in range(8863)]).to(torch.int64)
        #test_entitities= torch.Tensor([1,5,7,9,11]).to(torch.int64)
        test_ent_embed= self.entity_embedding(test_entitities)
        #self.ent_embed=test_ent_embed
        #print('shape test***',test_ent_embed) #, test_ent_embed.shape
        ##print('shape test***',test_ent_embed)
        ### add test
        return loss, new_state, F.log_softmax(scores), label_action, chosen_relation,test_ent_embed
