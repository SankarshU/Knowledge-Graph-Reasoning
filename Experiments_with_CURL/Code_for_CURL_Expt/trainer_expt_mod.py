##### simple changes made for Experiment context inclusion of a Ticket in the troubleshooting are included ####
############################## Added for Experiment context inclusion troubleshooting ##############################
 ### <code within this block  is the addition>
############################## Added for Experiment context inclusion troubleshooting ##############################
...
class Trainer(object):
	...
	def calc_reinforce_loss(self, all_loss, all_logits, cum_discounted_reward, decaying_beta, baseline):
		...
		# [original batch_size * num_rollout, T]
	...
	def entropy_reg_loss(self, all_logits):  # control diversity
		# [original batch_size * num_rollout, max_num_actions, T]
		all_logits = torch.stack(all_logits, dim=2)
		entropy_loss = - \
		    torch.mean(
		        torch.sum(torch.mul(torch.exp(all_logits), all_logits), dim=1))  # scalar
		return entropy_loss

	def calc_cum_discounted_reward(self, rewards):

		running_add = torch.zeros([rewards.size(0)]).to(
		    self.device)  # [original batch_size * num_rollout]
	...
	def calc_cum_discounted_reward_credit(self, approx_credits, entity_rewards, cluster_rewards):
		...
		running_add = torch.zeros([num_instances]).to(
		    self.device)  # [original batch_size * num_rollout]
		cum_disc_reward = torch.zeros([num_instances, self.path_length]).to(
			self.device)  # [original batch_size * num_rollout, T]
		cum_disc_reward[:,
		...
		for t in reversed(range(1, self.path_length)):
			# approx_credits[t].to(self.device) * cluster_rewards
			running_add = self.gamma * running_add + \
			    cum_disc_reward[:, t] + cluster_rewards
	...
	def regularization_cluster(self, cluster_scores, e_agent_pred_clusters):
		...
		reg_loss = torch.nn.functional.cross_entropy(
		    cluster_scores, e_agent_pred_clusters)
		return reg_loss
	...
	def cluster_entity_embeddings_sim_reg(self):
		...
		for cls, ents in self.cluster2ent_.items():
			...
			ent_emb = [self.e_agent.entity_embedding(
			    torch.LongTensor([ent]).to(self.device)) for ent in ents]
			ent_emb = torch.cat(ent_emb, dim=0)
			ent_emb = torch.mean(ent_emb, dim=0)
			cls_emb = self.c_agent.cluster_embedding(
			    torch.LongTensor([cls]).to(self.device))
	...
	def train(self):
		...
		for entity_episode, cluster_episode in self.train_environment.get_episodes(self.batch_counter):
			...
			next_possible_entities = torch.tensor(
			    entity_state['next_entities']).long().to(self.device)

			# range_arr = torch.arange(self.batch_size * self.num_rollouts).to(self.device)
			# original batch_size * num_rollout, 1-D, (1...)
			prev_relation = self.e_agent.dummy_start_label.to(self.device)

			query_relation = entity_episode.get_query_relation()
			query_relation = torch.tensor(query_relation).long().to(self.device)
			current_entities_t = torch.tensor(
			    entity_state['current_entities']).long().to(self.device)
			...
			next_possible_clusters = torch.tensor(cluster_state['next_clusters']).long().to(
				self.device)  # original batch_size * num_rollout, max_num_actions
			prev_possible_clusters = torch.zeros_like(
			    next_possible_clusters).to(self.device)

			cluster_state_emb = torch.zeros(1, 2, self.batch_size * self.num_rollouts,
											self.e_agent.m * self.embedding_size).to(self.device)

			range_arr = torch.arange(
			    self.batch_size * self.num_rollouts).to(self.device)
			...
			current_clusters_t = torch.tensor(
			    cluster_state['current_clusters']).long().to(self.device)
			...
			for i in range(self.path_length):
				...
				# important !! switch to next state with new cluster
				cluster_state = cluster_episode.next_action(idx)
				prev_possible_clusters = next_possible_clusters.clone()
				next_possible_clusters = torch.tensor(
				    cluster_state['next_clusters']).long().to(self.device)
				current_clusters_t = torch.tensor(
				    cluster_state['current_clusters']).long().to(self.device)
				prev_cluster = chosen_relation.to(self.device)

				loss, entity_state_emb, logits, idx, chosen_relation,test_ent_embed = self.e_agent.step(
				...
				# important !! switch to next state with new entity and new relation
				entity_state, whether_e_agent_follows_c_agent = entity_episode(
				    idx, prev_cluster.cpu(), i)
				next_possible_relations = torch.tensor(
				    entity_state['next_relations']).long().to(self.device)
				next_possible_entities = torch.tensor(
				    entity_state['next_entities']).long().to(self.device)
				current_entities_t = torch.tensor(
				    entity_state['current_entities']).long().to(self.device)
				...
				entity_episode.get_stepwise_approximated_reward(
				    current_entities_t,	current_clusters_t, prev_entities)  # estimate the reward by taking each step
				prev_entities = current_entities_t.clone()
			...
			torch.nn.utils.clip_grad_norm_(
			    self.c_agent.parameters(), max_norm=self.grad_clip_norm, norm_type=2)
			self.optimizer.step()
			...
			if self.batch_counter % self.eval_every == 0:
				...
				self.path_logger_file_ = self.path_logger_file + \
				    "/" + str(self.batch_counter) + "/paths"
		...
		print('****embed****', np.array(test_ent_embed.detach()))
		np.save('/home/jupyter/EXPTS/DKGR-master/dtac_embed.npy', np.array(test_ent_embed.detach()))        
	def test(self, beam=False, print_paths=False, save_model=True, auc=False):
		...
		with torch.no_grad():
			...
			for entity_episode, cluster_episode in tqdm(self.test_environment.get_episodes(0)):
				...
				beam_probs = torch.zeros(
				    (temp_batch_size * self.test_rollouts, 1)).to(self.device)
				...
				range_arr = torch.arange(
				    temp_batch_size * self.test_rollouts).to(self.device)
				...
				for i in range(self.path_length):
					...
					loss, entity_state_emb, test_scores, test_action_idx, chosen_relation,_ = self.e_agent.step(
					...
					if beam:
						...
						test_action_idx = x
						c_idx = c_x
						chosen_relation = entity_state['next_relations'][np.arange(
						    temp_batch_size * k), x]
					...
					# important !! switch to next state with new cluster
					cluster_state = cluster_episode.next_action(c_idx)
					...
					self.log_probs += test_scores[np.arange(
					    self.log_probs.shape[0]), test_action_idx.cpu().numpy()]
				if beam:
				...
				self.log_probs = np.reshape(
				    self.log_probs, (temp_batch_size, self.test_rollouts))
				...
				final_reward_20 = 0
				AP = 0
				ce = entity_episode.state['current_entities'].reshape(
				    (temp_batch_size, self.test_rollouts))
				se = entity_episode.start_entities.reshape(
				    (temp_batch_size, self.test_rollouts))
				for b in range(temp_batch_size):
					...
					if answer_pos == None:
						...
						AP += 1.0 / ((answer_pos + 1))
					if print_paths:
						qr = self.train_environment.entity_grapher.rev_relation_vocab[
			...
			auc /= total_examples
			if save_model:
				if all_final_reward_20 >= self.max_hits_at_10:
					self.max_hits_at_10 = all_final_reward_20
					torch.save(self.e_agent.state_dict(),
					           self.model_dir + "e_model" + '.ckpt')
					torch.save(self.c_agent.state_dict(),
					           self.model_dir + "c_model" + '.ckpt')
					# self.save_path = self.model_dir + "model" + '.ckpt'

			if print_paths:
				logger.info("[ printing paths at {} ]".format(
				    self.output_dir + '/test_beam/'))
				for q in paths:
					j = q.replace('/', '-')
...
################
    entity2vec,relation2vec=None,None
...
if __name__ == '__main__':
    ...
    print(options,type(options),'start1')
    
    options = read_pretrained_embeddings(options) # adds pretrained embeddings dictionary  to #'concept:agriculturalproductcookedwithagriculturalproduct': array
    ...
    print('default embed',options['pretrained_embeddings_entity'])
    ############################## Start:Added for Experiment context inclusion troubleshooting ##############################
    ### This addition is to create one hop neighbours for each Tickets(T) before training of path based raesoning to provide dynamic context
    ### Modified agent.py leverages options['tt_embeddings_entity'] and options['dtac_embed'] initialized here
    pathx='/home/jupyter/EXPTS/DKGR-master/datasets/data_preprocessed/DTAC/vocab/'
    js='entity_vocab.json'
    rs='relation_vocab.json'
    with open(pathx+js) as f:
        ent = json.load(f)
    entrev={v:k for k,v in ent.items()}
    with open(pathx+rs) as f:
        rel = json.load(f)
    relrev={v:k for k,v in rel.items()}
    tta=[]
    for x in ent:
        if x.find('INC')>-1:
            tta.append(ent[x])
    len(tta)
    pathy='/home/jupyter/EXPTS/DKGR-master/datasets/data_preprocessed/DTAC/'
    filename=pathy+'graph.txt'
    ent_1n_dct=defaultdict(list)
    with open(filename) as file:
        for line in file:
            ...
            e,r,o=sx
            #break
            ent_1n_dct[ent[e]].append([rel[r],ent[o]])
            #break           

    ent_1n_dct_tt={k:v for k,v in  ent_1n_dct.items() if k in tta}
    print(len(ent_1n_dct), len(ent_1n_dct_tt))
    dlist=[0  for i in range(50)]
    emblist=[]
    for key in options['entity_vocab'].keys():
        val=options['entity_vocab'][key]
        dlist=[0  for i in range(50)]
        if val in ent_1n_dct_tt:
            rent,oent=zip(*ent_1n_dct_tt[val])
            #if len(
            oent=list(oent)
            oent.append(val)
            #print(oent)
            dlist[0:len(oent)]=oent

        if len(dlist) !=50:
            print(dlist,'err',len(dlist))
        emblist.append(dlist)
    emblistlookup=np.array(emblist)
    print(emblistlookup.shape)
    options['tt_embeddings_entity'] = emblistlookup
    options['dtac_embed'] = True
    #### changes for DTAC
    ############################## END:Added for Experiment context inclusion troubleshooting ##############################
    ...
    if not options['load_model']:
        ...
        #trainer.entity_embed=None
        trainer.train()
        print(trainer.ent_embed, 'yahoo')
